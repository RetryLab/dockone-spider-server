...
Account.prototype.auth = function(req, res, next) {
    var uuid = (req.cookies.uuid || req.headers.authorization || '' || '569f393d0e885806005c9ad6|758274BA0A86393DECBD4E8416D83BE2');
    if (!uuid)
...
Profile.prototype.BuildScript = function(profile, machine) {
    var period = profile.cpu_use > 1? profile.cpu_use* 1000 : 1000;
    return 'docker pull kerngven/dockerone-crawler-client &&'
            + ' docker run'
            + ' -d -m ' + Math.floor((machine.mem_size || 1 * profile.mem_use || 1)* 1000) + 'm'
            + ' --cpu-period=' + period
            + ' --cpu-quota=1000'
            + ' --restart=always'
...
Profile.prototype.update = function(req, res) {
    ...
    var id = req.params.id;
    var body = req.body;
    var machine_id= body.machine_id;
    try {
        this.validate(body);
        body.updated = new Date();
    } catch (e) {
    ...
    exports.machine.mongo.findOne({
        _id: mongo.ObjectId(machine_id)
    }, function(error, machine) {
        if (error)
            res.status(500).end();
        else {
            body._id= id;
            body.user_id= machine.user_id;
