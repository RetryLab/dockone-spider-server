    ...
    exports.machine.mongo.findOne({
        ...
        if (error)
            ...
            body.script = 'docker pull kerngven/dockerone-crawler-client\n\
                            docker run --restart always -d -g ' + (machine.mem_size * body.mem_use) + 'g --cpu-period=' + period + ' --cpu-quota=' + quota + ' \
                            -e auth_id=' + auth_id + ' -e hostname=' + config.hostname + ' kerngven/dockerone-crawler-client';
            this.mongo.insertOne(body, function(error, result) {
                if (error)
                    res.status(500).end();
                else {
