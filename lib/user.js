...
Profile.prototype.create = function(req, res) {
    ...
    exports.machine.mongo.findOne({
        _id: mongo.ObjectId(profile.machine_id)
    }, function(error, machine) {
        if (error)
            res.status(500).end();
        else {
            var period = body.cpu_use * 1000;
            var quota = period * cpu_num_use;
            body.script = 'docker pull kerngven/dockerone-crawler-client\n\
                            docker run --restart always -d -g ' + (machine.mem_size * body.mem_use) + 'g  --name=test --cpu-period=' + period + ' --cpu-quota=' + quota + ' \
                            -e auth_id=' + auth_id + ' -e host=' + config.host + ' kerngven/dockerone-crawler-client';
            this.mongo.insertOne(body, function(error, result) {
                if (error)
                    res.status(500).end();
                else {
